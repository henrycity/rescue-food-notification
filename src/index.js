...
const K_MARKET_SELLO_MESSAGE =
  '{"t":"d","d":{"r":11,"a":"q","b":{"p":"/prod-real-thing-no-kidding/detailed/3955","h":""}}}';
const MAIL_SERVER = {
  host: 'smtp.gmail.com',
  port: 587,
  secure: false,
  ...
  try {
    ...
    if (message.d.b?.p === K_MARKET_SELLO_PATH && message.d.b.d.offersLeft) {
      ...
      if (numberOfOffersLeft < data.offersLeft) {
        ...
        let offersLeft = '';
        if (data.offers) {
          offersLeft = Object.values(data.offers)
            .filter((offer) => offer.unitsLeft)
            .map((offer) => getOfferNameAndDescription(offer))
            .join(',');
        } else {
          offersLeft = Object.entries(data)
            .filter(([key]) => key.includes('offers/'))
            .map(([key, value]) => getOfferNameAndDescription(value))
            .join(',');
        }
    ...
    console.error(error);
    sendMail({ MAIL_SERVER, mail: 'Error' }).catch(console.error);
  }
});

const getOfferNameAndDescription = (offer) => `${offer.name}/${offer.unitsLeft} units`;
